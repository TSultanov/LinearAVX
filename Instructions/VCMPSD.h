...
class VCMPSD : public CompilableInstruction<VCMPSD> {
public:
    VCMPSD(uint64_t rip, uint8_t ilen, xed_decoded_inst_t xedd) : CompilableInstruction(rip, ilen, xedd) {}

    static const inline InstructionMetadata Metadata = {
        .iclass = XED_ICLASS_VCMPSD,
        .operandSets = {
            { 
                .vectorLength = 64,
    ...
    void implementation(bool upper, bool compile_inline) {
        ...
        if (mode <= 0x07) {
            map3opto2op(upper, [&](xed_encoder_operand_t const& op0, xed_encoder_operand_t const& op1) {
                cmpsd(op0, op1, operands[3].toEncoderOperand(upper));
            });
        } else
        ...
        if (mode == 0x16) {
            ...
            map3opto2op(upper, [&](xed_encoder_operand_t const& op0, xed_encoder_operand_t const& op1) {
                cmpsd(op0, op1, xed_imm0(0x06, 8));
            });
        } else {
            debug_print("VCMPSD: Unsupported mode %llx\n", mode);
            exit(1);
        }
