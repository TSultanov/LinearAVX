...
public:
    VADDPS(uint64_t rip, uint8_t ilen, xed_decoded_inst_t xedd) : CompilableInstruction(rip, ilen, xedd) {}

    static const inline InstructionMetadata Metadata = {
        .iclass = XED_ICLASS_VADDPS,
        .operandSets = {
            { 
                .vectorLength = 128,
    ...
    void implementation(bool upper, bool compile_inline) {
        if (operands[0].reg() == operands[1].reg()) {
            ...
            withPreserveXmmReg(operands[1], [=]() {
                addps(operands[1].toEncoderOperand(upper), operands[2].toEncoderOperand(upper));
